...
class Segmentation:
    ...
    def doSegmentation(self,filename=None,size_thresh=None):
        ...
        for x in range(0,i-2,1):
            # self.flag=1  # assuming its a blank row
            # ty=1
            # while (ty<j-2):
            #     if (self.img[x][ty]==0 or self.img[x][ty]==50):
            #         self.flag=0 # no it is not
            #         break
            #     ty+=1
        ...
        self.chars=self.chars.reshape((-1,4))
        self.duplicateChars()
